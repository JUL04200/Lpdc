...
async function doWebPayment({ amountCents, orderNumber, name, pickupTime, cartSummary, subtotal, discount, total, returnURL, cancelURL, notificationURL }) {
  ...
  // L'ordre des champs doit respecter exactement la sequence du schema
  // Monext (payment, returnURL, cancelURL, order, notificationURL,
  // privateDataList, languageCode, buyer, ...) : un ordre different
  // provoque un rejet generique ("System internal error") sans detail.
  const body =
...
async function getWebPaymentDetails(token) {
  const body = "<impl:getWebPaymentDetailsRequest>" + "<version>25</version>" + `<token>${xmlEscape(token)}</token>` + "</impl:getWebPaymentDetailsRequest>";
