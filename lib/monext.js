...
async function createSession({ amountCents, orderNumber, name, pickupTime, cartSummary, subtotal, discount, total, returnURL, notificationURL }) {
  const ref = buildOrderRef(orderNumber);

  const body = {
    pointOfSaleReference: "1",
    // Le point de vente a 2 contrats CB actifs (CB_MONEXT_3DS et
    // VISAMASTER_MONEXT_3DS) : Monext refuse de choisir automatiquement,
    // il faut preciser explicitement lequel utiliser.
    paymentMethod: { contractNumber: "CB_MONEXT_3DS" },
    order: {
      reference: ref,
      amount: amountCents,
      currency: "EUR",
