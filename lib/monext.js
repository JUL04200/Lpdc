...
async function createSession({ amountCents, orderNumber, name, pickupTime, cartSummary, subtotal, discount, total, returnURL, notificationURL, contractNumber }) {
  ...
  const body = {
    ...
    // Restreint aussi les moyens de paiement AFFICHES sur la page hebergee
    // a ce seul contrat (sinon Monext montre tous les contrats actifs du
    // point de vente en onglets, avec un choix d'onglet par defaut qui
    // ignore contractNumber).
    selectedContractList: [chosenContract],
    order: {
      reference: ref,
      amount: amountCents,
      currency: "EUR",
